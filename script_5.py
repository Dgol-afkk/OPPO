"""
Модуль для управления данными о недвижимости.

Содержит классы для представления объектов недвижимости, чтения данных из файла,
сервисной логики обработки и консольного пользовательского интерфейса.
"""
...
class PropertyReader(abc.ABC):
    ...
    def read(self) -> List[Property]:
        """Читает файл и возвращает список валидных объектов недвижимости. """
...
class PropertyService:
    ...
    def get_all_sorted_by_date(self) -> List[Property]:
        return sorted(
            self._properties, key=lambda p: p.reg_date, reverse=True
        )
...
class ConsoleUI:
    ...
    def _get_price_range(self) -> Tuple[int, int]:
        min_p = self._get_valid_int_input("Введите минимальную стоимость: ")

        while True:
            max_p = self._get_valid_int_input("Введите максимальную стоимость: ")
            if max_p >= min_p:
                return min_p, max_p
            print("Ошибка: макс. стоимость не может быть меньше минимальной.")
