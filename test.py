...
from script import Property, FilePropertyReader, PropertyService, PropertyReader
...
class TestPropertyValidation:
    ...
    def test_invalid_cost_type(self):
        with pytest.raises(ValueError):
            Property(cost="100", owner="Test", reg_date=datetime.now())
    ...
    def test_invalid_date_type(self):
        with pytest.raises(TypeError):
            Property(cost=100, owner="Owner", reg_date="2022-01-01")
...
class TestParser:
    ...
    def test_valid_parse(self, reader):
        ...
        result = reader._parse_line(raw_line)
    ...
    def test_invalid_date_format(self, reader):
        raw_line = 'Недвижимость: Владелец: "Петров", Дата: 2022-99-99, Стоимость: 100 руб.'
        assert reader._parse_line(raw_line) is None

    def test_missing_data(self, reader):
        raw_line = 'Недвижимость: Дата регистрации: 2021.11.11, Стоимость: 3200000 руб.'
        assert reader._parse_line(raw_line) is None

    def test_garbage_string(self, reader):
        assert reader._parse_line("") is None
        assert reader._parse_line("Просто набор слов") is None
